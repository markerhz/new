...
}
{
  ...
  rows[0] = '00103123';
  setTypes(board, rows);
  const hint = match.findPossibleMove();
  ok(hint && hint.from.isAdjacentTo(hint.to), 'หา Hint เป็นคู่ช่องติดกันที่สลับได้');
...

{
  ...
  fx.novaWave(256, 256, () => 0.5);
  ok(fx.novaWaves.length === 1 && fx.particles.length === 36, 'Nova สร้างคลื่น + ประกายดูดแสงครบ');
  fx.update(700);
  ok(fx.novaWaves.length === 0, 'คลื่น Nova หมดอายุแล้วถูกลบ');
}

{
  const fx = new Effects();
  fx.cometSweep(128, 128, 'h', () => 0.5);
  ok(fx.cometSweeps.length === 1 && fx.cometSweeps[0].axis === 'h', 'Comet สร้างการกวาดตามแกนที่กำหนด');
  fx.update(600);
  ok(fx.cometSweeps.length === 0, 'เอฟเฟกต์ Comet หมดอายุแล้วถูกลบ');
}

{
  const fx = new Effects();
  fx.bombBlast(64, 64, () => 0.5);
  ok(fx.bombBlasts.length === 1 && fx.particles.length === 28, 'Bomb สร้างวงแรงกระแทก + เศษประกายครบ');
  fx.update(500);
  ok(fx.bombBlasts.length === 0, 'วงแรงกระแทก Bomb หมดอายุแล้วถูกลบ');
}

{
  const fx = new Effects();
  fx.rockets.push({ progress: 0 });
  ok(fx.busy, 'จรวดที่กำลังบินทำให้ Effects อยู่ในสถานะ busy');
}
{
  const fx = new Effects();
...
console.log('--- GemArt: จานสี/แผนที่ไฟล์ asset pack (v0.6.0) ---');
{
  const { GemArt } = await import('../js/engine/GemArt.js');
  // v0.6.0: GemArt เปลี่ยนจากคำนวณทรงเจมสด (procedural) มาโหลดสไปรต์ PNG จริงจาก asset pack
  // ส่วนที่ยังทดสอบแบบ pure-data ได้ใน Node คือ "แผนที่ข้อมูล" (PALETTE/SPECIAL_FILES/ANIM)
  // ส่วนวาดจริง (buildSprites/drawGem) ต้องพึ่ง document/Image — ทดสอบด้วยตาในเบราว์เซอร์เท่านั้น
  ok(GemArt.PALETTE.length === 6, 'PALETTE มีครบ 6 สี');
  ok(GemArt.PALETTE.every((p) => /^#[0-9A-Fa-f]{6}$/.test(p.m)), 'ทุกสีเป็น hex ที่ถูกต้อง (.m)');
  const files = GemArt.PALETTE.map((p) => p.file);
  ok(files.every((f) => typeof f === 'string' && f.length > 0), 'ทุกเจมมีชื่อไฟล์ asset (.file)');
  ok(new Set(files).size === 6, 'ชื่อไฟล์เจมไม่ซ้ำกันเลย (' + files.join(',') + ')');
  ok(/^#[0-9A-Fa-f]{6}$/.test(GemArt.OUTLINE), 'OUTLINE เป็น hex ที่ถูกต้อง');
  ok(GemArt.ANIM.length === 6 && GemArt.ANIM.every((a) => a.floatAmp > 0 && a.floatSpeed > 0 && a.glowSpeed > 0),
    'ทุกเจมมีจังหวะลอย/เต้นของตัวเอง (ANIM ครบ 6, ค่าเป็นบวกทั้งหมด)');

  const expectedSpecials = { bomb: 'bomb', nova: 'nova', cometH: 'comet_h', cometV: 'comet_v', rocket: 'rocket' };
  ok(JSON.stringify(GemArt.SPECIAL_FILES) === JSON.stringify(expectedSpecials),
    'SPECIAL_FILES ผูกชื่อ candy.special กับไฟล์ asset ถูกต้องครบ 5 ชนิด');
}
